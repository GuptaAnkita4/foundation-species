...
# Ensure WVclass factor order and no negative/zero issues for logs
normalize_glm_dat <- function(df) {
  ...
  df %>%
    mutate(
      ...
      # safety: keep originals; use log1p for y to avoid -Inf
      log_rich = log1p(richness),
      log_abun = log1p(abundance)
    )
}
...
fit_nb <- function(df, response) {
  # response is "richness" or "abundance"
...
predict_grid <- function(mod, df, term_name = "logArea") {
  ...
  # Sequence over the focal term and all WVclass levels
  ...
  # Provide a grid value; random effects are excluded with re.form=NA so a dummy is fine
  newdata$grid <- df$grid[1]
  
  # Predict on response scale; exclude REs
...
anova_sig_label <- function(mod) {
  ...
  # try to pull p-value for interaction term if present
  rn <- rownames(a); term <- which(grepl("logArea:factor\\(WVclass\\)", rn))
  p <- if (length(term)) a$`Pr(>Chisq)`[term] else NA_real_
  if (is.na(p)) return("p = NA")
  if (p < 0.001) "p < 0.001" else if (p < 0.01) "p < 0.01" else if (p < 0.05) "p < 0.05" else "p > 0.05"
}
...
make_wv_plot <- function(pred_data, obs_data, season_label, yvar_logcol,
  ...
  # Limits include BOTH observed and prediction domain
  ...
  ggplot() +
    # POINTS (observed; log1p)
    geom_jitter(
      data = obs_data,
      aes(x = logArea, y = .data[[yvar_logcol]], shape = point_group, fill = point_group),
      width = 0.1, alpha = 0.4, size = 2, stroke = 0.4, color = "black"
    ) +
    # LINES (predicted; use log1p to match y-axis transform)
    geom_line(
      data = pred_data,
      aes(x = x, y = log1p(predicted), color = line_group, group = line_group),
    ...
    scale_color_manual(values = line_vals, labels = wv_labels, name = "Lines: % Wetland Vegetation") +
...
# axis limits using log1p(y) (avoids -Inf)
...
p_a <- make_wv_plot(pred_rich_s,  s.glm.dat, "Summer", "log_rich", lab_rich_s,  "log1p(Richness)",  xlim_rich,  ylim_rich)
p_b <- make_wv_plot(pred_rich_w,  w.glm.dat, "Winter", "log_rich", lab_rich_w,  "log1p(Richness)",  xlim_rich,  ylim_rich) + labs(y = NULL)
p_c <- make_wv_plot(pred_count_s, s.glm.dat, "Summer", "log_abun", lab_count_s, "log1p(Abundance)", xlim_abund, ylim_abund)
p_d <- make_wv_plot(pred_count_w, w.glm.dat, "Winter", "log_abun", lab_count_w, "log1p(Abundance)", xlim_abund, ylim_abund) + labs(y = NULL)
...
# Save outputs
ggsave(here("Figures", "SAR_WV_combined_color.png"), combined_plot, width = 14, height = 5, dpi = 300)
ggsave(here("Figures", "SAR_WV_summer.png"), p_a, width = 6, height = 5, dpi = 300)
ggsave(here("Figures", "SAR_WV_winter.png"), p_b, width = 6, height = 5, dpi = 300)
...
# helper: extract fixed effects + CIs + anova + fit stats from glmmTMB
extract_glmmTMB <- function(mod, label) {
  ...
  av <- tryCatch({
    ...
    names(at) <- sub("Pr\\(>Chisq\\)", "p_value", names(at))
    at$model <- label
    at
  }, error = function(e) NULL)
  
  # fit stats (sigma is overdispersion-like for nbinom2)
  fit <- dplyr::tibble(
    model   = label,
    family  = fam_name,
    theta   = sm$sigma,
...
# combined index (handy overview: one row per model)
combine_interaction_p <- function(av, pattern = "logArea:factor\\(WVclass\\)") {
  ...
  if (!nrow(row)) return(NA_real_) else as.numeric(row$p_value[1])
}
...
# save prediction grids too (so figures can be rebuilt without refitting)
preds <- list(
...
# finally, save model objects (RDS bundle)
saveRDS(
  list(
    s_nb_rich  = s_nb_rich,
